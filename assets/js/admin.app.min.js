...
var mimicTrading = angular.module('mimicTrading', ['ui.router', 'ngAnimate', 'ngMessages', 'ngFileUpload', 'LocalStorageModule', 'ui.bootstrap', 'oc.lazyLoad', 'ngSanitize', 'angular-ladda', 'ui.mask']);
'use strict';
		...
		resolve: {
			deps: ['$ocLazyLoad', function ($ocLazyLoad) {
				return $ocLazyLoad.load({
					...
					files: ['/css/admin-login-style.css']
				});
			}]
		}
	}).state('dashboard', {
		...
		resolve: {
			deps: ['$ocLazyLoad', function ($ocLazyLoad) {
				return $ocLazyLoad.load({
					name: 'mimicTrading',
					insertBefore: '#ng_load_plugins_before', // load the above css files before a LINK element with this ID. Dynamic CSS files must be loaded between core and theme css files
					files: ['/global/plugins/morris/morris.css', '/global/plugins/morris/morris.min.js', '/global/plugins/morris/raphael-min.js', '/global/plugins/jquery.sparkline.min.js', '/pages/scripts/dashboard.min.js']
				});
			}]
		},
		authenticate: true
...
mimicTrading.controller('customerCtrl', ['$scope', '$state', 'RestSvr', 'loginSrv', '$rootScope', '$log', '$timeout', 'Upload', 'marketTradedSvr', 'appSvr', function ($scope, $state, RestSvr, loginSrv, $rootScope, $log, $timeout, Upload, marketTradedSvr, appSvr) {
  ...
  */
		if ($state.current.name === 'customers') {
			...
			TableAjax.init({
				url: 'user/listing/1',
				columns: [{ "data": "id", "orderable": false }, { "data": "first_name" }, { "data": "last_name" }, { "data": "email" }, { "data": "status" }, { "data": "date_joined" }, { "data": "action", "orderable": false }]
			});
		}
	});
...
mimicTrading.controller('customerEditCtrl', ['$scope', '$state', 'RestSvr', 'loginSrv', '$rootScope', '$log', '$timeout', 'Upload', 'appSvr', 'customer', function ($scope, $state, RestSvr, loginSrv, $rootScope, $log, $timeout, Upload, appSvr, customer) {
	$scope.$on('$viewContentLoaded', function () {
		/**
   * Initialize the jquery components when view contents loaded properly
   */
...
mimicTrading.controller('customerViewCtrl', ['$scope', 'customer', '$state', 'appSvr', function ($scope, customer, $state, appSvr) {
...
mimicTrading.controller('userCtrl', ['$scope', '$state', 'RestSvr', 'loginSrv', '$rootScope', '$log', '$timeout', 'Upload', 'marketTradedSvr', 'appSvr', function ($scope, $state, RestSvr, loginSrv, $rootScope, $log, $timeout, Upload, marketTradedSvr, appSvr) {
...
mimicTrading.controller('userEditCtrl', ['$scope', '$state', 'RestSvr', 'loginSrv', '$rootScope', '$log', 'trader', 'Upload', 'marketTradedSvr', 'appSvr', function ($scope, $state, RestSvr, loginSrv, $rootScope, $log, trader, Upload, marketTradedSvr, appSvr) {
	$scope.$on('$viewContentLoaded', function () {
		/**
   * Initialize the jquery components when view contents loaded properly
   */
...
mimicTrading.controller('userViewCtrl', ['$scope', '$state', 'RestSvr', 'loginSrv', '$rootScope', '$log', 'trader', 'Upload', '$location', 'appSvr', function ($scope, $state, RestSvr, loginSrv, $rootScope, $log, trader, Upload, $location, appSvr) {
	$scope.$on('$viewContentLoaded', function () {
		/**
   * Initialize the jquery components when view contents loaded properly
   */
...
mimicTrading.config(['$stateProvider', function ($stateProvider) {
  ...
  */
	var customerResolver = {
		customer: ['customerSvr', '$stateParams', function (customerSvr, $stateParams) {
			return customerSvr.getCustomerById($stateParams.id);
		}]
	};
		...
		resolve: {
			deps: ['$ocLazyLoad', function ($ocLazyLoad) {
				return $ocLazyLoad.load({
					name: 'mimicTrading',
					insertBefore: '#ng_load_plugins_before', // load the above css files before a LINK element with this ID. Dynamic CSS files must be loaded between core and theme css files
					files: ['/global/plugins/datatables/datatables.min.css', '/global/plugins/datatables/plugins/bootstrap/datatables.bootstrap.css', '/global/plugins/bootstrap-datepicker/css/bootstrap-datepicker3.min.css', '/global/plugins/datatables/datatables.all.min.js', '/global/plugins/bootstrap-datepicker/js/bootstrap-datepicker.min.js', '/global/scripts/datatable.min.js']
				});
			}]
		},
		authenticate: true
...
mimicTrading.config(['$stateProvider', function ($stateProvider) {
  ...
  */
	var traderResolver = {
		trader: ['traderSvr', '$stateParams', function (traderSvr, $stateParams) {
			return traderSvr.getTraderById($stateParams.id);
		}]
	};
...
mimicTrading.directive('notificationPanel', ['notificationSvr', '$location', '$rootScope', function (notificationSvr, $location, $rootScope) {
	return {
		restrict: 'A',
		link: function link(scope, elem) {
			notificationSvr.getNotificationList().then(function (_ref) {
				var records = _ref.records,
				    paging = _ref.paging;

				scope.notifications = records;
				return notificationSvr.getNotificationCount();
			}).then($rootScope.manageCount).catch(function (errors) {
...
mimicTrading.factory('customerSvr', ['RestSvr', function (RestSvr) {
    return {
        getCustomerById: function getCustomerById(id) {
            return RestSvr.post('user/view/' + id);
        }
    };
}]);
'use strict';
...
mimicTrading.factory('marketTradedSvr', ['RestSvr', function (RestSvr) {
    return {
        getValue: function getValue() {
            return RestSvr.post('market/traded/');
        },
        getTraderList: function getTraderList() {
            return RestSvr.post('trader/listing/').then(function (_ref) {
                var records = _ref.records;
                return records;
            });
        }
...
mimicTrading.factory('traderSvr', ['RestSvr', function (RestSvr) {
    return {
        getTraderById: function getTraderById(id) {
            return RestSvr.post('user/view/' + id);
        }
    };
}]);
